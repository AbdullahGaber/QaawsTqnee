...
class _promoDetailState extends State<promoDetail> {
  ...
  Widget build(BuildContext context) {
    ...
    var _grid = SingleChildScrollView(
      child: FutureBuilder(
        future: APIService().getProducts(tagId: Config.weekPromotion),
        builder: (ctx, AsyncSnapshot<List<ProductModel>> product) {
          if (product.hasData) {
            print('data tata');
            List<ProductModel> promoProduct = product.data.where((pro) {
              var discount = ((double.parse(pro.regularPrice) -
                      double.parse(pro.salePrice)) /
                  double.parse(pro.regularPrice) *
                  100);
              print('dicount is $discount');
              return discount.toInt() == widget.discountPercent;
            }).toList();
            print(promoProduct);
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  GridView.builder(
                    ...
                    primary: false,
                    itemBuilder: (ctx, i) =>
                        ItemGrid(promoProduct[i], widget.discountPercent),
...
class ItemGrid extends StatelessWidget {
  ...
  ProductModel item;
  final int percent;
  ItemGrid(this.item, this.percent);
  Widget build(BuildContext context) {
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        image: DecorationImage(
                            image: NetworkImage(item.images[0].src),
                            fit: BoxFit.cover)),
                  ),
                  Container(
                    height: 25.5,
